...
class MLClient:
    ...
    def start_child_process(self,cmd):
      list=cmd.split(' ')
      return Popen(list, stdout=PIPE, stderr=PIPE)

    def check_running_job(self,job):
      P=job['child_process']
      self.print_process_output(P)
      retcode=P.poll()
    ...
    def print_process_output(self,P):
      while True:
        output_stdout= P.stdout.readline()
        output_stderr = P.stderr.readline()
